...
class RRi:
    ...
    def plot(self, ax=None, detect_peaks=True, *args, **kwargs):
        fig = None
        if ax is None:
        ...
        if detect_peaks:
            peaks, _ = find_peaks(self.rri, prominence=10)
            peaks_to_time = [self.time[x] for x in peaks]
            print(peaks_to_time)
            #results_half = peak_widths(self.rri, peaks_to_time, rel_height=0.5)
            #results_full = peak_widths(self.rri, peaks_to_time, rel_height=1)
            plt.plot(peaks_to_time, self.rri[peaks], "x")
